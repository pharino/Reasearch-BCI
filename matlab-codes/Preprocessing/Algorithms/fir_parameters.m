function [ output_args ] = fir_parameters( varargin )
...
if nargin < 8 || (isempty(find(strcmp(varargin,'WindowFunction')))) || isempty(find(strcmp(varargin,'SamplingFrequency'))) || ...
    ...
    wf_str  = varargin{find(strcmp(varargin,'WindowFunction')) +1 };
    wfh     = str2func(wf_str);
    ...
    if isempty(find(strcmp(varargin,'WindowParameters')))
        ...
        wfpar   = varargin{find(strcmp(varargin,'WindowParameters')) +1 };
        i   = length(wfpar);
        switch i
            case 1
    ...
    %% ---------------------------[Optional]-----------------------------%%
    if varargin{find(strcmp(varargin,'Plot')) + 1} == 1
        %   Create plot view
        ...
        gcf;
        subplot(2,1,1);
        plot(freq,hdB,'LineWidth',1.5,'Color','k');
        xlim([ 0,50]);
        grid on;
        xlabel('Frequency[Hz]');
        ylabel('Amplitude(dB)');
        ...
        subplot(2,1,2);
        plot(freq,phi,'LineWidth',1.5,'Color','k');
        xlim([0 50]);
        ylim([-180 180]);
        grid on;
        xlabel('Frequency[Hz]');
