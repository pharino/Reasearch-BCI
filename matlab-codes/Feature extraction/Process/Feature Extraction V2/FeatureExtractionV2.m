function FeatureExtractionV2(varargin)
%   FeatureExtractionV2
%   Input:
...
Process.File.Path        = 'D:\CHUM Pharino Master 2011-2013\Master 2011-2013\Research Paper\Matlab code\Signal processing\Feature extraction\Process\Feature Extraction V2';
Process.File.Name{1}     = 'Feature Extraction V2-Input.txt';       % Input file
Process.File.Name{2}     = 'Feature Extraction V2-Output.txt';      % Output file
...
for File = 1:length(Process.File.Dataset{1,1}{1})
    ...
    Process.Data.Temp.CrossValidation.Partition         = [0.7 0.15 0.15];
    ...
    for tr = 1:Process.Data.Temp.STFT.DataSize(1)       % Loop by trial
        for ch = 1:Process.Data.Temp.STFT.DataSize(2)   % Loop by channel
            ...
            %   Time epoching
            Process.Data.Temp.STFT.Signal(tr,ch,:,:) = Process.Data.Temp.STFT.Data.Power(...
                Process.Data.Temp.STFT.FreqLimit(1):Process.Data.Temp.STFT.FreqLimit(2),...
                Process.Data.Temp.STFT.TimeLimit(1):Process.Data.Temp.STFT.TimeLimit(2));
        end
    ...
    %%  -------------------------------------------------------------------
    %   Calculate Fisher score for each cross-validation
    
    display('Calculating Fisher score....');
    %       For each fold of validation data
    Process.Data.Temp.FisherScore.SignalSize = Process.Data.Main.Information.SignalSize;
    Process.Data.Temp.FisherScore.Score = zeros(Process.Data.Main.Information.CrossValidation.FoldNumber,...
    ...
    progressbar;
    for Fold = 1:Process.Data.Main.Information.CrossValidation.FoldNumber
        
        %       Class seperation
        Process.Data.Temp.FisherScore.ClassLabel    = unique(Process.Data.Main.Information.class_output);
        Process.Data.Temp.FisherScore.IndexTrain    = Process.Data.Main.Information.CrossValidation.IndexTrain(Fold,:);
        ...
        progressbar(Fold/Process.Data.Main.Information.CrossValidation.FoldNumber);
    end
    
    %       Calculate average score
    
    Process.Data.Temp.FisherScore.ScoreMean         = mean(Process.Data.Temp.FisherScore.Score,1);
    [Process.Data.Temp.FisherScore.ScoreSort,...
        Process.Data.Temp.FisherScore.IndexSort]    = sort(Process.Data.Temp.FisherScore.ScoreMean, 'descend');
    
    Process.Data.Temp.FisherScore.ScoreMean         = Process.Data.Temp.FisherScore.ScoreMean(:);
    ...
    Process.Data.Main.Information.FisherScore.Score         = Process.Data.Temp.FisherScore.ScoreMean;
    ...
    %%  Save Data
    display('Saving data...');
    FisherScoreAndMap.FisherScore{File}   = Process.Data.Main.Information.FisherScore;
    FisherScoreAndMap.Map{File}           = Process.Data.Main.Information.Map3D;
    Save        =  Process.Data.Main;
    Process     = rmfield(Process,'Data');
    save(Process.File.Dataset{1,2}{1}{File},'-struct','Save','Signal','Information');
end
save('Fisher Score and 3DMap.mat','-struct','FisherScoreAndMap','FisherScore','Map');
