...
Analysis.Save       = {'T-uniform-XOR.mat',...
    'T-single point-XOR.mat',...
    'T-two points-XOR.mat'};
...
for S = 1:length(Analysis.Save)
    ...
    GA.View.X(1,:) = linspace(0,1,100);
    GA.View.X(2,:) = linspace(0,1,100);
    for i = 1:length(GA.View.X(1,:))
        for k = 1:length(GA.View.X(2,:))
            GA.View.Y(i,k) =  xorStructure(GA.View.X(1,i),...
                GA.View.X(2,k),...
                0.5,...
                0.5,...
                0.1);
    ...
    for T = 1:Analysis.Iteration
        %%  -----------------------------------------------------------------------
        %   GA parameters
        GA.Parameters.FF    = @xorStructure;
        GA.Parameters.NPar  = 2;
        GA.Parameters.MaxIt = 500;
        GA.Parameters.PSize = 20;
        ...
        for i = 1:GA.Parameters.PSize
            GA.Chromosome.CPop(i,:) = decodepop(0,1,GA.Parameters.PNBit,GA.Chromosome.Pop(i,:));
            GA.Chromosome.Fitness(i)= feval(GA.Parameters.FF,...
                GA.Chromosome.CPop(i,1),...
                GA.Chromosome.CPop(i,2),...
                0.5,...
                0.5,...
                0.1);
        end
        ...
        for i = 1:GA.Parameters.MaxIt
            ...
            for k = 1:GA.Parameters.PSize
                GA.Chromosome.CPop(k,:) = decodepop(0,1,GA.Parameters.PNBit,GA.Chromosome.Pop(k,:));
                GA.Chromosome.Fitness(k)= feval(GA.Parameters.FF,...
                    GA.Chromosome.CPop(k,1),...
                    GA.Chromosome.CPop(k,2),...
                    0.5,...
                    0.5,...
                    0.1);
            end
            ...
            axis([0 GA.Parameters.PSize, 0 5]);
            xlabel('Individual');
            ...
            axis([0 GA.Parameters.MaxIt 0 5]);
            xlabel('Iteration');
